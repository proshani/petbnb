  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        scrolled ? "bg-charcoal/96 backdrop-blur-md shadow-2xl py-3" : "bg-transparent py-6"
      }`}
    >
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        {/* Logo */}
        <button onClick={() => scrollTo("hero")} className="flex items-center gap-2 group">
          <span className="font-serif text-2xl font-bold text-white tracking-widest group-hover:text-gold transition-colors duration-300">
            LUXE
          </span>
          <div className="flex flex-col ml-1 hidden sm:flex">
            ...
            <span className="text-white/40 text-[9px] tracking-[0.2em] uppercase leading-tight">
              Real Estate
            </span>
          </div>
        </button>
